...
;(function (root, factory) {
  ...
  JL.Renders = {
    box: {
      tpl: [
        '<section class="box" id="tris__commentBox">',
          '<div class="com-avatar"><img id="tris__loginAvatar" src="/img/unsigned_avatar.jpg" alt="avatar"></div>',
          '<div class="com-text">',
            '<div class="main">',
              '<textarea class="text-area-edited show" id="tris__editBox" placeholder="欢迎评论！"></textarea>',
              '<div class="text-area-preview" id="tris__previewBox"></div>',
            '</div>',
            '<div class="switch">',
              '<div class="switch-item on" id="tris__editSwitcher" onclick="tris.Actions.editPreviewSwitch(\'edit\')">编辑</div>',
              '<div class="switch-item" id="tris__previewSwitcher" onclick="tris.Actions.editPreviewSwitch(\'preview\')">预览</div>',
            '</div>',
            '<div class="button" onclick="tris.Actions.postComment()">提交</div>',
          '</div>',
        '</section>'
      ].join(''),
      update: function () {
        ...
        // 默认头像路径 /img/tris.jpg
        $('tris__loginAvatar').src = userInfo.avatar_url || '/img/unsigned_avatar.jpg';
      }
    },
    list: {
      tpl: [
        '<section class="list-wrap" id="tris__commentList">',
      ...
      update: function (page, comments, list, callback) {
        ...
        if (comments === 0) {
            ...
            item = [
              '<li class="item">',
                ...
                '<div class="user-comment">',
                  '<div class="user-comment-header" id="tris__comment_' + list[i].id + '_reactions">',
                    ...
                    '<span class="like" onclick="tris.Actions.like(' + list[i].id + ')">点赞</span>',
                    '<span class="like-num">' + list[i].reactions.heart + '</span>',
                    '<span class="reply" onclick="tris.Actions.reply(\'' + list[i].user.login + '\', \'' + (list[i].body_html || list[i].body).replace(/<[^>]+>|\s|[\r\n]/g, ' ') + '\')">回复</span>',
                  '</div>',
          ...
          if (allPages === 1) {
            ...
            for (var i = 1; i <= allPages; i++) {
              if (i === page) {
                ...
                pageItem = '<a href="javascript: tris.Actions.pageJump(' + i + ');" class="item">' + i + '</a>';
              }
              pageList.push(pageItem);
            }
            if (page !== 1) {
              pageList.unshift('<a href="javascript: tris.Actions.pageJump(' + (page - 1) + ');" class="item">上页</a>');
            }
            if (page !== allPages) {
              pageList.push('<a href="javascript: tris.Actions.pageJump(' + (page + 1) + ');" class="item">下页</a>');
            }
          } else if (allPages > perNavPageMaxSize) {
            if (page <= perNavPageMaxSize) {
              ...
              pageList.push('<a href="javascript: tris.Actions.pageJump(' + (page + 1) + ');" class="item">下页</a>');
              pageList.push('<a href="javascript: tris.Actions.pageJump(' + allPages + ');" class="item">末页</a>');
              ...
              pageList.push('<a href="javascript: tris.Actions.pageJump(1);" class="item">首页</a>');
              pageList.push('<a href="javascript: tris.Actions.pageJump(' + (page - 1) + ');" class="item">上页</a>');
          ...
          html = [
            '<header class="list-header">总共 <span class="comments-num" id="tris__commentsNum">' + JL.issueComments + '</span> 条评论</header>',
            '<ul class="list">',
              htmlList.join(''),
            '</ul>',
            '<div class="page-nav">',
        ...
        $('tris__commentList').innerHTML = html;
      ...
      reactionUpdate: function (commentId, reactions) {
        ...
        for (var i = 0, len = reactions.length; i < len; i++) {
          if (userId === reactions[i].user.id) {
            ...
            addClass($('tris__comment_' + commentId + '_reactions').getElementsByClassName('like')[0], 'liked');
            $('tris__comment_' + commentId + '_reactions').getElementsByClassName('like')[0].innerHTML = '已赞';
            break;
          }
        }
      },
      addOne: function (data) {
        ...
        var item = [
          ...
          '</div>',
          '<div class="user-comment">',
            '<div class="user-comment-header" id="tris__comment_' + data.id + '_reactions">',
              '<span class="post-name">' + data.user.login +  '</span>',
              '<span class="post-time">' + formatDate('yyyy-MM-dd hh:mm', new Date(data.created_at)) + '</span>',
              '<span class="like" onclick="tris.Actions.like(' + data.reactions.heart + ')">点赞</span>',
              '<span class="like-num">' + data.reactions.heart + '</span>',
              '<span class="reply" onclick="tris.Actions.reply(\'' + data.user.login + '\', \'' + (data.body_html || data.body).replace(/<[^>]+>|\s|[\r\n]/g, ' ') + '\')">回复</span>',
            '</div>',
            '<div class="user-comment-body">' + (data.body_html || data.body) + '</div>',
          '</div>'
        ].join('');
        oLi.innerHTML = item;
        var oUl = $('tris__commentList').getElementsByTagName('ul')[0];
        if (oUl) {
          oUl.insertBefore(oLi, oUl.firstChild);
          $('tris__commentsNum').innerHTML = JL.issueComments + 1;
        } else {
          $('tris__commentList').innerHTML = [
            '<header class="list-header">总共 <span class="comments-num" id="tris__commentsNum">' + (JL.issueComments + 1) + '</span> 条评论</header>',
            '<ul class="list">',
              '<li class="item">',
                item,
              '</li>',
    ...
    signBar: {
      tpl: [
        '<div class="sign-bar" id="tris__commentSignBar">',
        '</div>'
      ].join(''),
      update: function () {
        ...
        if (token && userInfo) {
          ...
          html = [
            ...
            '<span class="sign-link" onclick="tris.Actions.signOut()">退出</span>'
          ].join('');
        } else {
          html = [
        ...
        $('tris__commentSignBar').innerHTML = html;
      }
    },
    tips: {
      tpl: '<section class="tips clearfix" id="tris__comment_tips">注：评论支持 markdown 语法！</section>',
      update: function () {
        var userInfo = localStorage.getItem(constants.USER_INFO_KEY);
        var handler = '';
        // 如果文章还没关联 issue 并且登录账号是自己时
        if (userInfo && JSON.parse(userInfo).login === JL.options.owner && JL.issueNumber === 0) {
          handler = '<a href="javascript: tris.Actions.createIssue();" class="init" title="文章关联 issue">初始化评论</a>';
        }
        $('tris__comment_tips').innerHTML = handler + '注：评论支持 markdown 语法！';
      }
    },
    ...
    loading: {
      ...
      remove: function () {
        var oLoading = $('tris__loadingMask');
  ...
  JL.Actions = {
    ...
    editPreviewSwitch: function (flag) {
      if (flag === 'edit') {
        removeClass('tris__previewSwitcher', 'on');
        addClass('tris__editSwitcher', 'on');
        removeClass('tris__previewBox', 'show');
        addClass('tris__editBox', 'show');
      } else {
        removeClass('tris__editSwitcher', 'on');
        addClass('tris__previewSwitcher', 'on');
        removeClass('tris__editBox', 'show');
        addClass('tris__previewBox', 'show');
        var text = $('tris__editBox').value.trim();
        // 安全转义
        ...
        if (text) {
          JL.Requests.markdown({
            ...
            $('tris__previewBox').innerHTML = res;
          });
        } else {
          $('tris__previewBox').innerHTML = '';
        }
      }
    },
    postComment: function () {
      ...
      var body = $('tris__editBox').value.trim();
      // 安全转义
    ...
    like: function (commentId) {
      var oLiked = $('tris__comment_' + commentId + '_reactions').getElementsByClassName('liked');
      var oLike = $('tris__comment_' + commentId + '_reactions').getElementsByClassName('like')[0];
      var oNum = $('tris__comment_' + commentId + '_reactions').getElementsByClassName('like-num')[0];
      var accessToken = localStorage.getItem(constants.ACCESS_TOKEN_KEY);
      var userInfo = localStorage.getItem(constants.USER_INFO_KEY);
      if (oLiked.length) {
        return false;
